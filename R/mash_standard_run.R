...
#'     discounted by mashr.
load_g2m_df <- function(path, numSNPs){
  top_set <- readRDS(file.path(path, paste0("effects_",
...
mash_standard_run <- function(path, list_input = NA, numSNPs = NA,
  ...
  if(!is.na(list_input[1])){
    ...
    list_input <- load_g2m_df(path = path, numSNPs = numSNPs)
  } else stop("A path to gapit2mashr rds output, or 'numSNPs', the number of SNPs used in gapit2mashr, needs to be specified to load the data frames needed for mash.")
