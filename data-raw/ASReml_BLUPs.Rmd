...
```{r}
SYasr <- readRDS("../data-raw/ASReml/SY_Reviewer_Model_ASReml.rds")
SYold <- readRDS("../data-raw/ignore/SY_BLUPs_kinship.rds")
```
...
as.data.frame(SYasr$coefficients$fixed) %>%
  ...
  filter(!is.na(Earliest_Year_CDBN) & bu > -10000) %>%
  ggplot(aes(x = bu, y = value)) +
  geom_point()
